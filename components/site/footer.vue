<template>
    <div class="mt-auto rounded-t-3xl bg-accent-2 px-20 py-8 pt-12 text-gray-300 footer z-[10000]">
        <div class="flex flex-row">
            <div class="flex flex-row">
                <a href="#" class="duration-100 hover:text-primary">Foire aux questions</a>
                <div class="mx-8">•</div>
                <a href="#" class="duration-100 hover:text-primary">Politique de confidentialité</a>
                <div class="mx-8">•</div>
                <a href="#" class="duration-100 hover:text-primary">Mentions légales</a>
                <div class="mx-8">•</div>
                <a href="#" class="duration-100 hover:text-primary">Conditions Générales de Vente</a>
                <div class="mx-8">•</div>
                <a href="#" class="duration-100 hover:text-primary">Conditions Générales d'Utilisation du site</a>
            </div>
            <div class="flex flex-row ml-auto">
                <a href="#" class="mx-4">
                    <IconFacebook class="h-8 w-8 fill-white duration-100 hover:fill-primary" />
                </a>
                <a href="#" class="mx-4">
                    <IconInstagram class="h-8 w-8 fill-white duration-100 hover:fill-primary" />
                </a>
                <a href="#" class="mx-4">
                    <IconTwitter class="h-8 w-8 fill-white duration-100 hover:fill-primary" />
                </a>
            </div>
        </div>
        <div class="text-center">
            &copy; Géo Move - Tous droits réservés
        </div>
    </div>
</template>

<style>
.footer {
    translate: 0 -3rem;
}
</style>